...
import { BookOpen, CalendarCheck, ChevronRight, ClipboardList, GraduationCap, LayoutDashboard, Megaphone, Menu, X } from "lucide-react";
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900 lg:grid lg:grid-cols-[240px_minmax(0,1fr)]">
    ...
    <div className="min-w-0"><header className="flex min-h-20 items-center justify-between gap-3 border-b border-slate-200 bg-white px-5 py-4 sm:px-8"><div><p className="text-[10px] font-bold uppercase tracking-[.15em] text-blue-700">{data.currentYear?.name ?? "No current school year"}</p><p className="mt-1 font-bold">{data.school.name}</p></div><AccountSignOut /></header>
      <main id="teacher-content" className="mx-auto max-w-[1400px] space-y-6 p-4 sm:p-8"><div><p className="text-xs text-slate-500">Teaching</p><h1 className="mt-1 text-2xl font-bold">{current.label}</h1></div>
        ...
        {current.id === "classes" && <div className="space-y-5">{data.classes.map(row => <Card key={row.id} title={`${data.gradeLevels.find(grade => grade.id === row.gradeLevelId)?.name ?? "Grade"} / ${row.name}`}><p className="mb-3 text-sm text-slate-500">{data.homeroomClassIds.includes(row.id) ? "Homeroom teacher" : `Subject teacher: ${allowedSubjects(row.id).map(item => item.name).join(", ")}`}</p><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b border-slate-200 text-slate-500"><th className="py-2">Student</th><th>Student number</th><th>Status</th><th>Safety information</th></tr></thead><tbody>{roster(row.id).map(student => <tr key={student.id} className="border-b border-slate-100"><td className="py-3 font-medium">{student.firstName} {student.lastName}</td><td>{student.studentNumber}</td><td>{words(student.status)}</td><td className="min-w-72 py-3"><StudentSafety studentId={student.id} safety={data.safety} /></td></tr>)}</tbody></table>{!roster(row.id).length && <p className="py-4 text-slate-500">No active students enrolled.</p>}</div></Card>)}</div>}
        {current.id === "attendance" && <Card title="Morning roll call"><div className="grid gap-3 sm:grid-cols-2"><Select label="Homeroom class" name="classId" choices={homeClasses.map(row => ({ id: row.id, name: row.name }))} value={homeClasses.some(row => row.id === classId) ? classId : homeClasses[0]?.id ?? ""} onChange={setClassId} /><form onSubmit={event => { event.preventDefault(); router.push(`/?section=attendance&date=${encodeURIComponent(str(new FormData(event.currentTarget), "date"))}`); }}><Field label="Date" name="date" type="date" defaultValue={date} /><button type="submit" className={`${secondary} mt-2`}>Load date</button></form></div>
          {homeClasses.length === 0 && <p className="mt-4 text-sm text-slate-500">Attendance is available to homeroom teachers.</p>}
          {homeClasses.length > 0 && <><div className="mt-4 divide-y divide-slate-100">{roster(homeClasses.some(row => row.id === classId) ? classId : homeClasses[0].id).map(student => { const id = homeClasses.some(row => row.id === classId) ? classId : homeClasses[0].id; const session = data.sessions.find(row => row.classId === id); const record = data.records.find(row => row.sessionId === session?.id && row.studentId === student.id); return <form key={student.id} className="grid items-end gap-3 py-3 md:grid-cols-[1fr_160px_1fr_1fr_auto]" onSubmit={event => submit(event, form => ({ kind: "attendance", classId: id, sessionDate: date, studentId: student.id, status: str(form, "status") as "present", reason: str(form, "reason"), correctionReason: str(form, "correctionReason") }))}><strong className="text-sm">{student.firstName} {student.lastName}</strong><Select label="Status" name="status" choices={["present", "absent", "late", "excused"].map(value => ({ id: value, name: words(value) }))} value={record?.status ?? "present"} /><Field label="Reason" name="reason" required={false} defaultValue={record?.reason ?? ""} /><Field label="Correction reason" name="correctionReason" required={false} /><button className={primary} disabled={pending || session?.status === "locked"}>Save</button></form>; })}</div><button className={secondary} disabled={pending || !roster(homeClasses.some(row => row.id === classId) ? classId : homeClasses[0].id).length} onClick={() => save({ kind: "attendance_submit", classId: homeClasses.some(row => row.id === classId) ? classId : homeClasses[0].id, sessionDate: date })}>Submit roll call</button></>}
        </Card>}
