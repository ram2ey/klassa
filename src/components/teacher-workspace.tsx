    ...
    <div className="min-w-0"><header className="flex min-h-20 items-center justify-between gap-3 border-b border-slate-200 bg-white px-5 py-4 sm:px-8"><div><p className="text-[10px] font-bold uppercase tracking-[.15em] text-blue-700">{data.currentYear?.name ?? "No current school year"}</p><p className="mt-1 font-bold">{data.school.name}</p></div><AccountSignOut /></header>
      <main id="teacher-content" className="mx-auto max-w-[1400px] space-y-6 p-4 sm:p-8"><div><p className="text-xs text-slate-500">Teaching</p><h1 className="mt-1 text-2xl font-bold">{current.label}</h1></div>
        ...
        {current.id === "attendance" && (() => {
          const isAttendanceLocked = attendanceSession?.status === "locked" || data.terms.some(t => t.isLocked && t.academicYearId === data.currentYear?.id && date >= t.startsOn && date <= t.endsOn);
          ...
          {attendanceClassId && <><p className="mt-4 text-sm text-slate-600">{isAttendanceLocked ? "Session is locked. This term or session is closed." : attendanceSession ? `Session ${words(attendanceSession.status)}.` : "No attendance recorded for this session yet."} Mark every active student before submitting.</p>
          {isAttendanceLocked && <p role="alert" className="mt-2 text-xs font-semibold text-amber-800 bg-amber-50 p-2 border border-amber-200">Attendance for this period or date is closed and locked. Marks cannot be saved.</p>}
          <div className="mt-4 divide-y divide-slate-100">{roster(attendanceClassId).map(student => { const record = data.records.find(row => row.sessionId === attendanceSession?.id && row.studentId === student.id); return <form key={`${attendanceClassId}-${attendancePeriod}-${student.id}`} className="grid items-end gap-3 py-3 md:grid-cols-[1fr_160px_1fr_1fr_auto]" onSubmit={event => submit(event, form => ({ kind: "attendance", classId: attendanceClassId, sessionDate: date, period: attendancePeriod, studentId: student.id, status: str(form, "status") as "present", reason: str(form, "reason"), correctionReason: str(form, "correctionReason") }))}><strong className="text-sm">{student.firstName} {student.lastName}</strong><Select label="Status" name="status" choices={["present", "absent", "late", "excused"].map(value => ({ id: value, name: words(value) }))} value={record?.status ?? "present"} /><Field label="Reason" name="reason" required={false} defaultValue={record?.reason ?? ""} /><Field label="Correction reason" name="correctionReason" required={false} /><button className={primary} disabled={pending || isAttendanceLocked}>Save</button></form>; })}</div><button className={secondary} disabled={pending || isAttendanceLocked || !roster(attendanceClassId).length} onClick={() => save({ kind: "attendance_submit", classId: attendanceClassId, sessionDate: date, period: attendancePeriod })}>Submit {attendancePeriod === "morning_roll_call" ? "roll call" : words(attendancePeriod)}</button></>}
        </Card>;
        })()}
        {current.id === "gradebook" && (() => {
          const isAssessmentTermLocked = selectedAssessment ? !!data.terms.find(t => t.id === selectedAssessment.termId)?.isLocked : false;
          return <div className="space-y-5"><Card title="Create assessment"><form className="grid gap-3 sm:grid-cols-2" onSubmit={event => submit(event, form => ({ kind: "assessment", classId: str(form, "classId"), termId: str(form, "termId"), subjectId: str(form, "subjectId"), categoryId: str(form, "categoryId"), title: str(form, "title"), maxScore: Number(str(form, "maxScore")), dateDue: str(form, "dateDue") }))}><Select label="Class" name="classId" choices={data.classes.map(row => ({ id: row.id, name: row.name }))} value={selectedClassId} onChange={value => { setClassId(value); setSubjectId(""); }} /><Select label="Term" name="termId" choices={data.terms.map(row => ({ id: row.id, name: row.isLocked ? `${row.name} (Closed & Locked)` : row.name }))} /><Select label="Subject" name="subjectId" choices={allowedSubjects(selectedClassId).map(row => ({ id: row.id, name: row.name }))} value={subjectId} onChange={setSubjectId} /><Select label="Category" name="categoryId" choices={data.categories.filter(row => row.subjectId === subjectId).map(row => ({ id: row.id, name: `${row.name} (${row.weight}%)` }))} /><Field label="Title" name="title" /><Field label="Maximum score" name="maxScore" type="number" min={1} max={1000} defaultValue={100} /><Field label="Due date" name="dateDue" type="date" /><button className={primary} disabled={pending || !selectedClassId}>Create assessment</button></form>{!data.categories.length && <p className="mt-3 text-sm text-amber-700">Ask your school administrator to set up assessment categories first.</p>}</Card>
          <Card title="Enter grades"><Select label="Assessment" name="assessmentId" choices={data.assessments.map(row => {
            const term = data.terms.find(t => t.id === row.termId);
            return { id: row.id, name: `${row.title} · ${subjectName(row.subjectId)} · ${words(row.status)}${term?.isLocked ? " (Closed & Locked)" : ""}` };
          })} value={activeAssessmentId} onChange={setAssessmentId} />{selectedAssessment && <><p className="mt-3 text-sm text-slate-500">Maximum score {selectedAssessment.maxScore}. Changes to published scores require a correction reason.</p>
