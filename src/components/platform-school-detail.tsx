...
export function PlatformSchoolDetail({ data }: { data: Data }) {
  ...
  function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        await updatePlatformSchoolAction({ organizationId: school.id, name, timezone });
        setMessage("School details saved."); router.refresh();
      } catch { setError("School details could not be saved. Check the name and timezone."); }
    });
  }
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <main className="mx-auto max-w-6xl space-y-6 p-5 lg:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-semibold uppercase tracking-widest text-blue-700">{school.slug}</p><h1 className="mt-1 text-3xl font-bold">{school.name}</h1><p className="mt-2 text-sm text-slate-500">Created {new Date(school.createdAt).toLocaleDateString()} · {school.timezone}</p></div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="border border-slate-200 bg-white"><Heading title="School profile" description="The tenant ID remains fixed so existing login names keep working." />
          <form onSubmit={save} className="space-y-4 p-5"><label className="block text-sm font-semibold">School name<input required minLength={2} maxLength={180} value={name} onChange={event => setName(event.target.value)} className="mt-1 block h-10 w-full border border-slate-300 px-3 font-normal" /></label>
            <label className="block text-sm font-semibold">Timezone<input required value={timezone} onChange={event => setTimezone(event.target.value)} className="mt-1 block h-10 w-full border border-slate-300 px-3 font-normal" /></label>
        ...
        <section className="border border-slate-200 bg-white"><Heading title="Access control" description="School suspension is reversible and recorded in the audit log." />
          <div className="space-y-4 p-5"><p className="text-sm text-slate-600">{school.suspendedAt ? `Suspended ${new Date(school.suspendedAt).toLocaleString()}. School accounts cannot sign in.` : "Active school accounts can sign in. Suspension signs them out and blocks access until reactivation."}</p>
            {school.suspendedAt
