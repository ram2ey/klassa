    ...
    <div className="min-w-0">
      ...
      <main id="school-content" className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-8">
        ...
        {current.id === "students" && <div className="space-y-6"><section className={panelStyle}>
          ...
          <DataTable caption="Student directory" headers={["Student", "Student number", "Current class", "Guardians", "Status", "Actions"]} rows={data.students.filter(student => matches(student.firstName, student.lastName, student.studentNumber)).map(student => {
            ...
            return { key: student.id, cells: [<strong key="name">{student.firstName} {student.lastName}</strong>, student.studentNumber, studentClass ? `${gradeName(studentClass.gradeLevelId)} / ${studentClass.name}` : "Not assigned",
              ...
              editButton("student", `Edit ${student.firstName} ${student.lastName}`, { id: student.id, studentNumber: student.studentNumber, firstName: student.firstName, lastName: student.lastName, dateOfBirth: student.dateOfBirth, status: student.status, classId: enrollment?.classId ?? "" })] };
...
function RecordEditor({ editor, data, onClose, onSaved }: { editor: Editor; data: SchoolAdminData; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  const fields: Record<Editor["kind"], Field[]> = {
    student: [{ name: "studentNumber", label: "Student number", required: true, max: 50 }, { name: "firstName", label: "First name", required: true, max: 100 }, { name: "lastName", label: "Last name", required: true, max: 100 }, { name: "dateOfBirth", label: "Date of birth", type: "date", required: true }, { name: "status", label: "Status", options: ["pending", "active", "withdrawn", "graduated"].map(value => ({ value, label: words(value) })) }, { name: "classId", label: "Class in current year", required: !editor.values?.id, options: [{ value: "", label: editor.values?.id ? "Keep current placement" : "Choose a class" }, ...data.classes.filter(item => item.academicYearId === year?.id).map(item => ({ value: item.id, label: `${data.grades.find(grade => grade.id === item.gradeLevelId)?.name} / ${item.name}` }))], hint: "Add grades and classes in Classes & grades if none are available." }],
