...
import { db } from "@/db";
import { academicYears, classes, organizations, reportCards, reportCardSubjectGrades, studentGuardians, students, subjects, teacherClassAssignments, terms } from "@/db/schema";
import { requireAccount, requireGuardian, requireStaff } from "@/lib/action-access";
...
export default async function ReportCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isGuardian = user.role === null && !user.isPlatformAdmin;
  const guardian = isGuardian ? await requireGuardian() : null;
  const actor = isGuardian ? null : await requireStaff(["school_admin", "teacher"]);
  const [card] = await db.select().from(reportCards).where(eq(reportCards.id, id));
  if (!card) notFound();
  const org = card.organizationId;
  if (guardian) {
    if (card.status !== "published") notFound();
    const guardianIds = guardian.guardians.filter(link => link.organizationId === org).map(link => link.id);
    if (!guardianIds.length) notFound();
    const [legalLink] = await db.select({ id: studentGuardians.id }).from(studentGuardians).where(and(
      eq(studentGuardians.organizationId, org), eq(studentGuardians.studentId, card.studentId),
      inArray(studentGuardians.guardianId, guardianIds), eq(studentGuardians.hasLegalResponsibility, true),
    )).limit(1);
    if (!legalLink) notFound();
  } else if (actor?.organizationId !== org) notFound();
  ...
  return <main className="mx-auto max-w-3xl bg-white p-6 text-slate-900 print:max-w-none print:p-0">
    <div className="mb-8 flex items-center justify-between gap-4 print:hidden"><Link href={guardian ? "/" : "/?section=reports"} className="text-sm font-semibold text-blue-700">← Back to report cards</Link><PrintReportButton /></div>
    ...
    <footer className="mt-12 border-t border-slate-300 pt-3 text-xs text-slate-500">Generated from the school records. {guardian ? "Published report card." : "Draft and approved copies are for internal review."}</footer>
  </main>;
}
