...
describe("deployment and action boundaries", () => {
  ...
  it("guards every fixture-backed server action before executing its body", () => {
    const liveActionFiles = ["guardian-portal-actions.ts", "invitation-actions.ts", "office-actions.ts", "password-actions.ts", "platform-account-actions.ts", "platform-audit-actions.ts", "platform-incident-actions.ts", "platform-school-actions.ts", "school-access-actions.ts", "school-workflow-actions.ts", "student-enrollment-actions.ts", "teacher-actions.ts"];
